...
const STATE_CONFIG = {
  normal: { label: "Steady rhythm", bg: "bg-teal-50", text: "text-teal-800", bar: "#5FB8A8", desc: "Movement pattern close to your usual baseline." },
  manic: { label: "Elevated rhythm", bg: "bg-amber-50", text: "text-amber-900", bar: "#E8B87A", desc: "Possible elevated activation — worth monitoring with your clinician." },
  depressed: { label: "Reduced rhythm", bg: "bg-slate-100", text: "text-slate-700", bar: "#94A8B8", desc: "Slower movement pattern — gentle check-in with yourself or your team may help." },
  pending: { label: "Collecting signal", bg: "bg-slate-50", text: "text-slate-600", bar: "#94a3b8", desc: "We are still gathering enough interaction samples for a stable read." }
};
  ...
  return (
    ...
    >
      <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${
                connected ? "bg-teal-50 text-teal-800" : "bg-amber-50 text-amber-900"
              }`}
            >
        ...
        {!connected && (
          <div className="rounded-xl border border-amber-100 bg-amber-50/80 px-3 py-2 text-xs leading-relaxed text-amber-950">
            The local analysis service is not running. When you are ready to test, start it with:
            <code className="mt-1 block rounded-lg bg-white/80 px-2 py-1 font-mono text-[11px] text-slate-700">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          {(eventCount > 0 || windows > 0) && (
            <div className="flex shrink-0 flex-col gap-1 rounded-xl border border-slate-200/60 bg-white/80 px-4 py-3 text-right sm:min-w-[140px]">
              {eventCount > 0 ? (
                <>
                  <span className="text-[11px] uppercase tracking-wide text-slate-500">Buffered events</span>
                  <span className="text-2xl font-semibold text-slate-800">{eventCount}</span>
                </>
              ) : null}
              {windows > 0 && (
                <>
                  ...
                  <span className={eventCount > 0 ? "text-xs text-slate-500" : "text-2xl font-semibold text-slate-800"}>
                    {eventCount > 0 ? `${windows} window${windows > 1 ? "s" : ""}` : windows}
                  </span>
                  <span className="text-xs text-slate-500">{anomalyPct.toFixed(0)}% flagged</span>
                </>
              )}
            </div>
          )}
