...
def run_chat_turn(
    ...
    *,
    ...
    top_k = top_k if top_k is not None else _env_int("RAG_TOP_K", 6)
    graph_hops = graph_hops if graph_hops is not None else _env_int("RAG_GRAPH_HOPS", 0)
    rerank_top_n = rerank_top_n if rerank_top_n is not None else _env_int("RAG_RERANK_TOP_N", 4)
