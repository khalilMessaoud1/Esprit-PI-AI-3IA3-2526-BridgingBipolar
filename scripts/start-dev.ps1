...
if ($missingVenv.Count -gt 0) {
    Write-Host "[1/3] Python venvs missing — running setup-python.ps1 (first run)..." -ForegroundColor Yellow
...
if (-not $SkipDeps) {
    Write-Host "[2/3] Docker (Postgres + Qdrant) + Prisma migrations + generate..." -ForegroundColor Yellow
    npm run dev:deps
    if ($LASTEXITCODE -ne 0) {
...
Write-Host "[3/3] Launching services in separate windows..." -ForegroundColor Yellow
Write-Host ""
...
if ($webPortBusy) {
    ...
    Write-Host "  Close the old 'BB Web' PowerShell window first, or use: .\scripts\start-dev.ps1 -WebPort 3002" -ForegroundColor Red
    Write-Host ""
}
